...
async function createTraefikMiddleware(jwtaName, jwtaNamespace, spec) {
...
async function deleteTraefikMiddleware(jwtaName, jwtaNamespace, spec) {
...
async function annotateIngress(jwtaName, jwtaNamespace, spec) {
...
async function createJwtAuthorizator(jwtaName, jwtaNamespace, spec) {
    //create configmap  
...
async function processAdd(jwtaObject) {
...
async function deleteJwtAuthorizator(jwtaName, jwtaNamespace, spec) {
    try {
        // recuperar config
        var configmapName = "jwta-authorizator-" + jwtaName + "-configmap";
...
function processDelete(jwtaObject) {
    var ns = jwtaObject.metadata.namespace;
    if (ns === undefined)
        ns = 'default';
    deleteJwtAuthorizator(jwtaObject.metadata.name, ns, jwtaObject);
}
async function modifyJwtAuthorizator(jwtaName, jwtaNamespace, spec) {
    //create configmap  
...
async function processModify(jwtaObject) {
    var namespace = jwtaObject.metadata.namespace;
    if (namespace === undefined)
        namespace = 'default';
    var ingress = jwtaObject.spec.ingress;
...
async function main() {
    try {
        ...
        watch.watch('/apis/jfvilas.at.outlook.com/v1/jwtauthorizators', {}, (type, obj) => {
